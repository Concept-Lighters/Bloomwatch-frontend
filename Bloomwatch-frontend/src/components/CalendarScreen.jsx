  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col max-w-md mx-auto">
      ...
      <div className="bg-calbgcolor px-4 py-3 flex items-center justify-between">
        <span className="text-3xl font-myFont font-semibold text-dashboardtextcolor">Mon, Aug 17</span>
      ...
      <div className="bg-calbgcolor px-4 py-2 flex items-center justify-between border-b border-gray-200">
        ...
        >
          <span className="text-sm font-medium font-myFont text-dashboardtextcolor">{monthNames[currentMonth]} {currentYear}</span>
          {isCalendarExpanded ? (
      ...
      <div className="bg-calbgcolor px-4 py-4 mb-4">
        ...
        <div className="grid grid-cols-7 gap-1">
          {calendarDays.map((dayObj, idx) => (
            <div key={`day-${idx}`} className="flex items-center justify-center h-10">
              {dayObj.day !== null && dayObj.day !== undefined ? (
                <button
                  className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-medium ${getDayStyle(dayObj)}`}
      ...
      <div className="bg-white px-4 py-3 mb-4">
        <h3 className="text-sm font-semibold text-dashboardheadcolor font-myFont mb-3">Task Types</h3>
        <div className="grid grid-cols-2 gap-3">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-green-500 rounded-full"></div>
            <span className="text-xs text-dashboardtextcolor font-myFont">Planting</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-blue-600 rounded-full"></div>
            <span className="text-xs text-dashboardtextcolor font-myFont">Fertilizer</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-purple-500 rounded-full"></div>
            <span className="text-xs text-dashboardtextcolor font-myFont">Pest Control</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-yellow-400 rounded-full"></div>
            <span className="text-xs text-dashboardtextcolor font-myFont">Harvest</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white px-4 py-3 mb-20">
        <h3 className="text-sm font-semibold font-myFont text-dashboardheadcolor mb-3">Upcoming Tasks</h3>
